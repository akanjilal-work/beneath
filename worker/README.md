...
**Every aircraft** from the OpenSky Network (`states/all`) is stored in R2 as `live/aircraft-global.json` and served at `/aircraft/global`. OpenSky does not answer requests from Cloudflare's network, so the `Aircraft overview` GitHub workflow fetches it every 15 minutes (`scripts/aircraft-overview.ts`, about one call per 15 minutes fits OpenSky's anonymous limit) and uploads it with wrangler. The app shows this worldwide overview when zoomed out and switches to live `/aircraft` queries up close. `/health` reports when the Kp and aircraft files were last written.
