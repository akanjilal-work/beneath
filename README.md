...
- **Go to the surface** from any card, or **Visit site** on a deposit, to fly down onto 3D terrain with satellite imagery, arriving at an angle like a map's 3D view. Close up, the imagery is Esri World Imagery (about 0.3 to 1 m per pixel almost everywhere); far views use Sentinel-2. Without an Esri key it falls back to 1 m USGS imagery over the US and 10 m elsewhere. Drag to look around, move with <kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> (or the on-screen pad), rise and descend with <kbd>E</kbd>/<kbd>Q</kbd>. The depth slider still drapes the data layers over the land.
- Turn on the **live layers above the surface**: about 16,000 **satellites** moving in real time (click one for its orbit), live **aircraft** near the view, and **cameras**: about 6,200 traffic cameras in Ontario, California and New York State, plus webcams worldwide near the view (provided by windy.com), each with its latest image.
